---
description: Gera o contrato OpenAPI (scalar/openapi.yaml) a partir do código real — detecta a stack e traduz rotas REST e/ou SvelteKit remote functions para OpenAPI 3.1, para o Scalar renderizar
argument-hint: "[escopo opcional: módulo/pasta/slug]"
allowed-tools: Bash(git:*), Bash(date:*), Bash(find:*), Bash(ls:*), Bash(grep:*), Bash(test:*), Bash(node:*), Bash(docker:*), Bash(curl:*), Read, Grep, Glob, Write, Edit, Task
---
...
- Data: !`date +%Y-%m-%d`
- Constitution: !`test -f docs/explanation/constitution.md && echo "presente" || echo "(ausente)"`
- Patterns: !`ls -1 docs/patterns/ 2>/dev/null || echo "(nenhum)"`
- Alvo do Scalar: !`(test -f scalar/openapi.yaml && echo "scalar/openapi.yaml") || (test -f docs/scalar/openapi.yaml && echo "docs/scalar/openapi.yaml") || echo "(Scalar não instalado)"`
- Remote functions: !`find . -path ./node_modules -prune -o -name '*.remote.ts' -print 2>/dev/null | head -20 || echo "(nenhuma)"`
- Endpoints SvelteKit: !`find . -path ./node_modules -prune -o -name '+server.ts' -print 2>/dev/null | head -20 || echo "(nenhum)"`
- Rotas Laravel: !`ls -1 routes/api.php routes/web.php 2>/dev/null || echo "(nenhuma)"`
